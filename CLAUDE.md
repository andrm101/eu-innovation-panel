...
| P10 — Audit | PASS (2026-09-25: seed audit, schema import check, vocab guard, content-hash verification all green on the host; two real bugs fixed along the way — p4_pca_scores.py had a false-positive-triggering `PCA(random_state=random_state)` passthrough the seed auditor couldn't statically resolve, and p8_profiles.py's archetype-profile template used the forbidden word "ranked". Also validated the sealed Docker reproduction path end-to-end (`docker build` + `make reproduce` + `make audit`) -- found and fixed 6 more real bugs there (wrong GDAL registry, missing hatchling wheel config, an RC-only Python 3.11 in Ubuntu's own apt repo, a gpg-agent-dependent PPA add that fails in this minimal image, an unsatisfiable pyampute version pin, and `uv` missing from the runtime stage), plus a stale Makefile `p4` target pointing at a `NotImplementedError` stub and a `make reproduce` that mounted no output directories so every containerized artifact was discarded on exit. Full pipeline (P1-P6) now runs inside the sealed container and reproduces the exact same GATE numbers as the host. expected_hashes.json covers data/gold/region_profiles_gold.parquet; profile PDFs and manuscript PDF are not yet rendered by this pipeline, so out of hash-audit scope for now — see REPRODUCE.md) |
